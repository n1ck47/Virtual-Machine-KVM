...
void vm_init(struct vm *vm, size_t mem_size)
{
	...
	memreg.userspace_addr = (unsigned long)vm->mem; // base address of the memory provided to the VM
...
int run_vm(struct vm *vm, struct vcpu *vcpu, size_t sz)
{
	...
	uint64_t memval = 0;
	unsigned int hostVirtualAddress = 0;
	
	unsigned int total_exits = 0;
	unsigned int in_exits = 0, out_exits = 0;
	...
	for (;;) {
		if (ioctl(vcpu->vcpu_fd, KVM_RUN, 0) < 0) {
			...
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT) out_exits++;
			if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_IN) in_exits++;
			total_exits++;
		}
		...
		switch (vcpu->kvm_run->exit_reason) {
			...
			case KVM_EXIT_IO:
				...
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
					...
					char *p = (char *)vcpu->kvm_run;
					uint32_t *pos = (uint32_t *)(p + vcpu->kvm_run->io.data_offset);
					printf("%" PRIu32 "\n", *pos);
					fflush(stdout);
					continue;
				}
				...
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
					...
					char *p = (char *)vcpu->kvm_run;
					uint32_t distance = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset);
					unsigned long gpa = (unsigned long)vm->mem; // base address of gpa
					char *str = (char *)(gpa + distance);
					printf("%s", str);
					fflush(stdout);
					continue;
				}
				...
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
					...
					char *kvm_run_ptr = (char *)vcpu->kvm_run;
					unsigned int *offset = (unsigned int *)(kvm_run_ptr + vcpu->kvm_run->io.data_offset);
					unsigned long hostVirtualAddressu = (unsigned long)vm->mem; // base address of gpa
					char *str = (char *)(hostVirtualAddressu + *offset);
					snprintf(str, 256, "IO in: %d\nIO out: %d\n", in_exits, out_exits);
					continue;
				}
				...
				if (vcpu->kvm_run->io.direction == KVM_EXIT_IO_OUT
					...
					char *p = (char *)vcpu->kvm_run;
					uint32_t distance = *(uint32_t *)(p + vcpu->kvm_run->io.data_offset); //gva
					
					if(distance > gpa_size) {
						printf("Invalid GVA\n");
					...
					unsigned long gpa = (unsigned long)vm->mem; // base address of gpa
					hostVirtualAddress = gpa + distance;
					continue;
				}
			...
			default:
				...
				printf("h %d %d %d\n", vcpu->kvm_run->io.direction, KVM_EXIT_IO_IN, KVM_EXIT_IO_OUT);
